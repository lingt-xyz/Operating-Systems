...
/**
 * Class BlockManager1 Implements character block "manager" and does twists with
 * threads.
 *
 * @author Serguei A. Mokhov, mokhov@cs.concordia.ca; Inspired by previous code
 *         by Prof. D. Probst
 *
 *         $Revision: 1.5 $ $Last Revision Date: 2017/02/08 $
 * 
 */
	...
	static class AcquireBlock extends BaseThread {
		...
		public void run() {
			...
			try {
				...
				System.out.println("Acq[TID=" + this.iTID + "]: Current value of top = " + soStack.getTop() + ".");
	...
	static class ReleaseBlock extends BaseThread {
		...
		public void run() {
			...
			try {
				...
				System.out.println("Rel[TID=" + this.iTID + "]: Current value of top = " + soStack.getTop() + ".");
			...
			try {
				for (int i = 0; i < siThreadSteps; i++) {
					...
					for (int s = 0; s < soStack.getSize(); s++)
						System.out.print((s == BlockManager5.soStack.getTop() ? "(" : "[")
								+ BlockManager5.soStack.getAt(s) + (s == BlockManager5.soStack.getTop() ? ")" : "]"));

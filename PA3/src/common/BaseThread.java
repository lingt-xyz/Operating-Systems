 ...
 *
 * An attempt to maintain an automatic unique TID (thread ID) among all the
 * derivatives and allow setting your own if needed. Plus some methods for the
 * sync exercises.
 *
 * $Revision: 1.2 $ $Last Revision Date: 2010/10/24 $
 *
...
public class BaseThread extends Thread {
	 ...
	 *
	 * @param poGroup
	 *            ThreadGroup to add this thread to
	 * @param pstrName
	 *            A string indicating human-readable thread's name
	 */
	...
	/**
	 * Sets internal TID and updates next TID on contruction time, so it's
	 * private.
	 */
	...
	/**
	 * Just a make up for the PHASE I to make it somewhat tangeable. Must be
	 * atomic.
	 */
	...
	/**
	 * Just a make up for the PHASE II to make it somewhat tangeable. Must be
	 * atomic.
	 */
	 ...
	 *
	 * Use to proceed to the phase II in the correct order. Must be atomic.
	 *
	 * @param pcIncreasingOrder
	 *            true if TIDs are in increasing order; false otherwise
	 *
	 * @return Returns true if if the TID of currently running thread matches
	 *         the turn, 'false' otherwise
	 */
	...
	/**
	 * Allows setting arbitratu turn value. Should be set only before the
	 * threads are started
	 */
	...
	/**
	 * Calls yield() several (4-40, pseudorandomly) times. Next to useless, but
	 * helps to mix up the execution of phases. Must NOT be atomic.
	 */

...
public class StackManager {
	...
	private static Semaphore MUTEX_RESOURCE = new Semaphore(0);
	private static Semaphore MUTEX_PRODUCER = new Semaphore(1);
	private static Semaphore MUTEX_CONSUMER = new Semaphore(1);
	// The main()
	...
	static class Consumer extends BaseThread {
		...
		public void run() {
			...
			for (int i = 0; i < StackManager.iThreadSteps; i++) {
				...
				while(MUTEX_RESOURCE.getValue() < 6){}
				StackManager.MUTEX_CONSUMER.P();
				try {
	...
	static class Producer extends BaseThread {
		...
		public void run() {
			...
			for (int i = 0; i < StackManager.iThreadSteps; i++) {
				...
				StackManager.MUTEX_RESOURCE.V();
				StackManager.MUTEX_PRODUCER.V();
			}
